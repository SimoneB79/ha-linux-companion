...
function loadDashboard() {
  ...
  mainWindow.webContents.on('did-navigate', (event, url) => {
    if (url.includes('/auth/authorize') || url.includes('/auth/login') || url.includes('/auth/token')) {
      authRetryCount++;
      log(`[WebView] HA login redirect detected (attempt ${authRetryCount}/${MAX_AUTH_RETRIES})`);
      if (authRetryCount > MAX_AUTH_RETRIES) {
        log('[WebView] Token appears invalid/revoked — showing login view instead of looping');
        authRetryCount = 0;
        loadLoginView();
        return;
      }
