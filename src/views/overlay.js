...
(function() {
  ...
  function scheduleBarHide() {
    if (barPinned) return;
    clearInterval(barHideTimer);
    barHideTimer = setInterval(function() { clearInterval(barHideTimer); if (!open) topbar.classList.add('hidden'); }, 5000);
  }
  ...
  topbar.addEventListener('mouseenter', function() { clearInterval(barHideTimer); topbar.classList.remove('hidden'); });
  ...
  function togglePanel() {
    ...
    barPinned = open;
    if (open) { topbar.classList.remove('hidden'); clearInterval(barHideTimer); loadPanelData(); }
